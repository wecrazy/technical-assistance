...
$(function () {
  ...
  var datatables_transactions = $(".datatables_transactions");
	if (datatables_transactions.length) {
    datatables_transactions.find('tbody').html(generateSkeletonLoadingHTML(10, 9));
		let d = new Date();
    var titleName = "Report Transactions"+d.getFullYear()+"-"+d.getMonth()+"-"+d.getDate();
		var column_maas =  Array.from({ length: 30 }, (_, i) => i);
		var datatables_transactions = datatables_transactions.DataTable({
			lengthMenu: [10, 25, 50, 100, 200, 500, 1000], 
			pageLength: 50, // Set the default page length
			serverSide: true, // Enable server-side processing
			ajax: {
				url: webPath+'tab-transaction/table',
				type: 'POST',
        data: function (d) {
      ...
      columnDefs: [
        ...
        buttons: [
          {
            extend: 'collection',
            filename: `Transaksi_Aktif_${timestampnow}`,
            ...
            buttons: [
              ...
              },
              {
                ...
                action: function (e, dt, button, config) {
                  fetch(webPath + 'tab-transaction/table.csv', {
                      method: 'GET',
                      headers: {
                  ...
                  .then(blob => {
                    ...
                    const filename = `Transasi_Aktif_${timestampnow}.csv`;
    ...
    $('.filter-trx').each(function() {
      $(this).on('change', function() {
        if ($(this).val() != "") {
            datatables_transactions.ajax.reload();
        }
      });
    });
	}
